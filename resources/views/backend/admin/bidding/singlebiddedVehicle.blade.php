...
@section('secton')
                        ...
                        <tbody>
                            ...
                            @forelse ($orders as $order)
                                <tr>
                                    ...
                                    <td class="requestedPrice">{{ $order->bid_price }}</td>
                                    <td class="requestedPrice">{{ $order->updated_at }}</td>
                                    <td>
                                        @if ($order->status == 0)
                                    ...
                                    <td><a class="badge badge-success"
                                            href="{{ route('orderdUserDetail', $order->user_id) }}">Dealer Details </a>
                                        <a class="badge badge-success"
                                            href="{{ route('orderdVehicleDetail', $order->vehicle_id) }}">Vehicle Details
                                        </a>
                                        <a class="badge badge-success"
                                            href="{{ route('orderdSellerDetail', $order->vehicle->user_id) }}">Seller Details
                                        </a>
                                        @if ($order->status == 0)
                                            <a class="badge badge-info"
                                                href="{{ route('approveBid', ['id' => $order->id, 'vid' => $order->vehicle->id]) }}">Approve
                                                Bid </a>
                                            <button type="button" class="btn btn-sm mt-2 btn-outline-primary block price"
                                                data-bs-toggle="modal" data-bs-target="#pricing"
                                                data-id="{{ $order->id }}">
                                                Update Price
                                            </button>
                                        @else
                                            <a class="badge badge-danger"
                                                href="{{ route('unassignBid', $order->id) }}">Unassign Bid</a>

                                            <button type="button" class="btn btn-sm mt-2  btn btn-outline-dark meeting"
                                                data-bs-toggle="modal" data-bs-target="#meeting"
                                                data-id="{{ $order->id }}">
                                                View Meeting Status
                                            </button>
                                        @endif
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('approveBidWithAdminUpdated') }}" method="POST">
                            @csrf
                            <input class="form-control" type="number" id="newPrice" name="updatedPrice"
    ...
    <div class="modal fade text-left " id="meeting" tabindex="-1" role="dialog" aria-labelledby="myModalLabel150"
        aria-hidden="true">
